...
export const Landing = () => {
  ...
  return (
    ...
    <div className="header text-center max-w-xs mx-auto">
      <img src={myLogoUrl} alt="logo" className="w-11/12 rounded-lg ml-2 my-3" />
    ...
    <div className="mx-auto mb-auto">
      ...
      <Link to="/cities">
        <img src={myButtonUrl} alt="citiesButton" className="w-24 rounded-lg m-4" />
      </Link>
    </div>
    <div className="mx-auto mb-10"> {/* Use mt-auto here to push this section to the bottom */}
    <p className="mb-6">Want to build your own MYtinerary?</p>
    <Link to="/logIn" className="text-white text-center rounded-lg bg-red-500 p-2 ms-4 ">
      Log In
    </Link>
    <Link to="/signUp" className="text-white text-center rounded-lg bg-red-500 p-2 ms-16">
      Sign Up
    </Link>
  </div>
    <div className="bg-red-500 p-4 text-white text-center fixed left-0 bottom-0 w-full">
    <img src={myHomeUrl} alt="homeButton" className="h-5 rounded-lg mx-auto" />
    </div>
  </div>
  )
}
